          ...
          {milestones.map((milestone, index) => (
            ...
            >
              <div className={`flex-1 ${index % 2 === 0 ? 'text-left' : 'text-right'}`}>
                <h3 className="text-2xl font-bold text-yellow-accent mb-2">{milestone.year}</h3>
                <h4 className="text-xl font-semibold mb-1 text-white">{milestone.title}</h4>
                <p className="text-white/80">{milestone.description}</p>
              </div>
              <div className="w-4 h-4 bg-yellow-accent rounded-full ring-4 ring-white/30 shadow-lg" />
